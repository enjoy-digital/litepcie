...
class USPPCIEPHY(LiteXModule):
    ...
    def __init__(self, platform, pads, speed="gen3", data_width=64, cd="sys",
        ...
        # Optional hard-IP debug interfaces. The external GT DRP clock is generated by the PCIe
        # hard IP; all other GT DRP signals are packed lane 0 first. The PCIe DRP clock is supplied
        # by the user. Both interfaces live outside the LitePCIe datapath and are disabled by
        # default so existing generated IP remains unchanged.
        self.with_ext_gt_drp = with_ext_gt_drp
        self.with_pcie_drp   = with_pcie_drp
        if with_ext_gt_drp:
            self.ext_gt_drp_clk  = Signal()
            self.ext_gt_drp_addr = Signal(10*len(pads.tx_p))
            self.ext_gt_drp_en   = Signal(len(pads.tx_p))
            self.ext_gt_drp_di   = Signal(16*len(pads.tx_p))
            self.ext_gt_drp_we   = Signal(len(pads.tx_p))
            self.ext_gt_drp_do   = Signal(16*len(pads.tx_p))
            self.ext_gt_drp_rdy  = Signal(len(pads.tx_p))
            self.config["en_ext_ch_gt_drp"] = True
        if with_pcie_drp:
            self.pcie_drp_clk  = Signal()
            self.pcie_drp_en   = Signal()
            self.pcie_drp_we   = Signal()
            self.pcie_drp_addr = Signal(10)
            self.pcie_drp_di   = Signal(16)
            self.pcie_drp_do   = Signal(16)
            self.pcie_drp_rdy  = Signal()
            self.config["en_pcie_drp"] = True

        # Native hard-IP status/control signals are in the PCIe user clock domain.
        self.cfg_err_cor           = Signal()
        self.cfg_err_nonfatal      = Signal()
        self.cfg_err_fatal         = Signal()
        self.cfg_local_error_valid = Signal()
        self.cfg_local_error       = Signal(5)
        self.link_training_enable  = Signal(reset=1)
        ...
        self.pcie_phy_params = dict(
            ...
            o_cfg_err_cor_out                  = self.cfg_err_cor,
            o_cfg_err_nonfatal_out             = self.cfg_err_nonfatal,
            o_cfg_err_fatal_out                = self.cfg_err_fatal,
            o_cfg_local_error_valid            = self.cfg_local_error_valid,
            o_cfg_local_error_out              = self.cfg_local_error,
        ...
        self.pcie_usp_phy_params = dict(
            ...
            i_cfg_link_training_enable          = self.link_training_enable,
        ...
        if with_ext_gt_drp:
            self.pcie_usp_phy_params.update(
                o_ext_ch_gt_drpclk  = self.ext_gt_drp_clk,
                i_ext_ch_gt_drpaddr = self.ext_gt_drp_addr,
                i_ext_ch_gt_drpen   = self.ext_gt_drp_en,
                i_ext_ch_gt_drpdi   = self.ext_gt_drp_di,
                i_ext_ch_gt_drpwe   = self.ext_gt_drp_we,
                o_ext_ch_gt_drpdo   = self.ext_gt_drp_do,
                o_ext_ch_gt_drprdy  = self.ext_gt_drp_rdy,
            )

        if with_pcie_drp:
            self.pcie_usp_phy_params.update(
                i_drp_clk  = self.pcie_drp_clk,
                i_drp_en   = self.pcie_drp_en,
                i_drp_we   = self.pcie_drp_we,
                i_drp_addr = self.pcie_drp_addr,
                i_drp_di   = self.pcie_drp_di,
                o_drp_do   = self.pcie_drp_do,
                o_drp_rdy  = self.pcie_drp_rdy,
            )
