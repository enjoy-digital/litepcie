...
def test_registered_read_controls(tmp_path, base_address):
    ...
    endpoint = SimpleNamespace(phy=SimpleNamespace(id=0x400),
    ...
    ports.update(('req_'+name, getattr(req, name)) for name in
        ('valid', 'ready', 'first', 'last', 'we', 'adr', 'len', 'first_be', 'last_be', 'tag', 'req_id', 'tc', 'attr', 'dat'))
    ports.update(('cmp_'+name, getattr(cmp, name)) for name in
        ('valid', 'ready', 'first', 'last', 'adr', 'len', 'byte_count', 'tag', 'req_id', 'cmp_id', 'tc', 'attr', 'dat', 'err'))
    ports.update(('wb_'+name, getattr(bus, name)) for name in ('adr', 'sel', 'cyc', 'stb', 'ack', 'we', 'dat_w'))
    ...
    inputs = {'clk', 'rst', 'cmp_ready', 'wb_ack'} | {'req_'+name for name in
        ('valid', 'first', 'last', 'we', 'adr', 'len', 'first_be', 'last_be', 'tag', 'req_id', 'tc', 'attr', 'dat')}
...
'''
    ...
    cases = [(1, be, 6) for be in (0, 1, 8, 5, 15)] + [(2, 12, 3), (5, 6, 8), (1024, 15, 15)]
    ...
    for tag, (count, first_be, last_be) in enumerate(cases):
        ...
        for index, be in enumerate(enables):
            ...
            expected_cmp.append((address+4*index+offset, remaining & 0xfff,
                ((word << 2)^0xa5a50000) & 0xffffffff, tag, 0x100, 0x400, 3, 2, 1, 1, 1, 0))
...
'''
        ...
        expected_wb.append((((write_address >> 2)+(base_address >> 2)) & address_mask, 15, 1, 0x9abcdef0))
...
'''
    ...
    assert result.returncode == 0, result.stdout+result.stderr
    wb = [tuple(int(value, 16) for value in line.split()[1:]) for line in result.stdout.splitlines() if line.startswith('WB ')]
    wb = [(adr, be, we, data if we else None) for adr, be, we, data in wb]
    cmps = [tuple(int(value, 16) for value in line.split()[1:]) for line in result.stdout.splitlines() if line.startswith('CMP ')]
